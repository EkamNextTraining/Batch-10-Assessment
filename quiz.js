...
//  GOOGLE SHEETS
// ══════════════════════════════════════════════════════════════════
function sendToSheets(timeStr, mcqScore, scenAnswered, codeAnswered, wasAuto) {
  ...
  if (!SCRIPT_URL || SCRIPT_URL.includes("YOUR_GOOGLE")) {
    line2.innerHTML = `Sending to Google Sheets... <span class="sub-warn">⚠️ SCRIPT_URL not set in quiz.js</span>`;
    line3.style.display = "block";
    line3.innerHTML = `<span class="sub-warn">Open quiz.js line 7 and replace YOUR_GOOGLE_APPS_SCRIPT_URL_HERE with your deployed Apps Script URL.</span>`;
    return;
  }
  ...
  for (let i = SCENARIO_START; i < SCENARIO_END; i++) {
    const q = QUESTIONS[i];
    const p = "Q" + String(i + 1).padStart(2, "0") + "_Scenario_";
  ...
  for (let i = MCQ_START; i < MCQ_END; i++) {
    ...
    const p   = "Q" + String(num).padStart(2, "0") + "_MCQ_";
    const si  = answers[i];
  ...
  for (let i = CODING_START; i < CODING_END; i++) {
    ...
    const p   = "Q" + String(num).padStart(2, "0") + "_Coding_";
  ...
  Object.assign(payload, scenarioCols, mcqCols, codingCols);

  // ── METHOD: hidden <form> POST — bypasses CORS completely ─────────
  // This is the only 100% reliable way to POST to Google Apps Script
  // from a GitHub Pages site without CORS errors.
  line2.innerHTML = "Sending to Google Sheets... <span class=\"sub-spin\">●</span>";

  try {
    // Create a hidden form pointing to the Apps Script URL
    const form = document.createElement("form");
    form.method  = "POST";
    form.action  = SCRIPT_URL;
    form.target  = "hidden_iframe"; // submit into invisible iframe — no page redirect
    form.style.display = "none";

    // Send the entire payload as one JSON field
    ...
    // Create the invisible iframe to receive the response
    ...
    // Listen for the iframe to load (= Apps Script responded)
    iframe.onload = function () {
      line2.innerHTML = "Sending to Google Sheets... <span class=\"sub-ok\">✓ Saved successfully</span>";
      line3.style.display = "block";
      line3.innerHTML = "<span class=\"sub-ok\">All responses written to Google Sheets. Check the sheet to confirm.</span>";
    };
    ...
    // Show optimistic message — iframe.onload confirms it
    setTimeout(function () {
      if (line2.innerHTML.includes("sub-spin")) {
        line2.innerHTML = "Sending to Google Sheets... <span class=\"sub-ok\">✓ Submitted</span>";
        line3.style.display = "block";
    ...
    line3.style.display = "block";
    line3.innerHTML = "<span class=\"sub-err\">Check that your SCRIPT_URL is correct and the Apps Script is deployed as a Web App with access set to Anyone.</span>";
  }
}
